...
- 实测操作：进入商品列表页，点击“全部商品”，在“商品编码”输入调用方传入的 `spuid`，点击查询按钮，并监听 `fetch/xhr` 请求。
...
| 商品编码 | `productListQueryReq.productIdList` | 调用方传入的 `spuid` |
| SKU编码 | `productListQueryReq.skuIdList` | `[<spuid>]` |
