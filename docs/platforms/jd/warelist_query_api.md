...
首个业务动作导航到商品列表页以加载京东安全 SDK，后续同店铺任务复用该有头会话；空闲十分钟或手动关闭时释放。服务在页面环境生成 H5ST 和 EID 并直接请求 SFF API。裸 `fetch` 会返回 `code: 312`，因此签名步骤不可省略。签名、EID、Cookie 和原始响应不得进入任务结果或日志。
